...
import { BILLING_INTERVAL_LABEL, FREE_TIER, PLANS, type Plan } from "@/lib/plans";

const CHECKOUT_NOTE =
  "Online checkout is being set up. Subscriptions aren't available to purchase on the website yet — we'll open it as soon as it's ready.";
...
export default function PricingCards() {
  ...
  const handlePlan = (plan: Plan) => {
    setNotice(null);
    if (user) {
      // No checkout exists yet — stay honest: don't fake a purchase. Show what
      // the click means today.
      setNotice(`“${plan.name}” can't be purchased on the website yet. Checkout is being set up.`);
      return;
    }
  ...
  return (
    <div>
      ...
      <p className="mx-auto mt-6 max-w-2xl text-center text-xs text-[var(--text-muted)]">
        ...
        month until cancelled. {CHECKOUT_NOTE}
      </p>

      {notice && (
        <div
          role="status"
          className="mx-auto mt-6 max-w-2xl rounded-2xl border border-[var(--accent-border)] bg-[var(--accent-dim)] px-5 py-4 text-sm text-[var(--text-primary)]"
        >
          {notice}
        </div>
      )}
