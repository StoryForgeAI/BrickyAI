...
The primary flow uses a code generated by the desktop app:

```
Desktop app (Tauri)
   │  1. Generates a high-entropy code and opens
   │     /auth/desktop?code=…  in the user's default browser
   ▼
Browser
   │  2. User clicks "Continue with Google" (existing Supabase Google OAuth)
   │  3. POST /api/auth/desktop/complete  { requestId: <same code>, code: <same code> }
   │     (Bearer access token) → the code is bound to the authenticated user
   │  4. Website shows "You're signed in!" then returns the browser to "/"
   ▼
Desktop app (Tauri)
   │  5. POST /api/auth/desktop/exchange  { "code": … }
...
A legacy `request_id` + `device_secret` flow (started by
`POST /api/auth/desktop/start`) remains supported for backwards compatibility.

Security properties:

- The **code never leaves the URL hash/unencrypted**: it is generated by the
  desktop app, appears in the browser URL once, and the database stores **only
  its SHA-256 hash**. It expires quickly (10 minutes total) and is **single-use** —
  redemption is a single atomic `UPDATE … WHERE status = 'bound'`, so exactly
  one exchange can ever win.
- Codes are validated server-side: printable ASCII only, no whitespace,
  8–512 characters; malformed codes are rejected before any query or insert.
- In the legacy flow the **`device_secret` is 256 bits of randomness**,
  generated server-side, returned exactly once to the desktop app, **never sent
  to the browser** and **never placed in a URL**.
- The request only becomes usable after a **validated Supabase session** binds
  it to a user id (`complete`).
...
### `POST /api/auth/desktop/start` — desktop app, pre-login (legacy)

Optional for the legacy `request_id` + `device_secret` flow; the primary
code-based flow does not call this endpoint. No auth required. Returns:
...
- Code-based flow (primary): body `{ "requestId": "<the code>", "code": "<the code>" }`.
  ...
  page load).
- Legacy flow: body `{ "requestId": "…" }` for a row created by `start`.
...
Code-based flow (primary) — send the code as a single credential field:

```json
{ "code": "…" }
```

The server also accepts `auth_code`, `authorization_code`, `token`,
`deviceSecret`, or `device_secret` for the same purpose. The row is looked up
by the SHA-256 hash of the code and registered as `pending` on first sight, so
a poll that beats the user to the browser gets `pending` instead of a terminal
error. Legacy body `{ "requestId": "…", "deviceSecret": "…" }` (from `start`)
is still accepted.
...
```rust
// 1. Generate a high-entropy code (e.g. 32+ random bytes, base64url).
let code: String = generate_code();

// 2. Open the browser, handing it the code.
let auth_url = format!(
    "https://bricky-ai.vercel.app/auth/desktop?code={}", code
);
...
- [x] No permanent tokens; codes are single-use with a 10-minute TTL (and the legacy `device_secret` is 256-bit random).
